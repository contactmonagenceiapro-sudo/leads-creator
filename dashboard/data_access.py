...
def get_stats() -> dict:
    try:
        leads_count = len(supabase.table("leads").select("id").execute().data)
...
def creer_ou_modifier_campagne(campagne: dict) -> dict:
    ...
    try:
        ...
        raise DataAccessError(f"Échec enregistrement campagne : {e}") from e
...
def dupliquer_campagne(nom_client: str, nouveau_nom_client: str, statut: str = "active") -> dict:
    ...
    try:
        ...
        raise DataAccessError(f"Échec duplication campagne : {e}") from e
    return {"campagne": resultat.data[0] if resultat.data else corps}
...
def renommer_campagne(nom_client: str, nouveau_nom_client: str) -> dict:
    ...
    try:
        ...
        raise DataAccessError(f"Échec renommage campagne : {e}") from e
    return {"campagne": resultat.data[0] if resultat.data else corps}
...
def creer_remboursement(demande: dict) -> dict:
    ...
    if contract_id:
        ...
        corps = {
            ...
            "client_final": client_final,
            "montant_centimes": demande.get("montant_centimes") or 0,
            "motif": motif,
            "type_remboursement": "avoir_commercial",
            "statut": "valide",
            "demande_par": demande.get("demande_par") or "operateur",
