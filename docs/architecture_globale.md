...
Généré le : 2026-09-04 22:00 UTC
...
- **Module 6 — Acquisition** : blocage de données levé (04/09/2026) — jusqu'ici `leads.source` ne prenait jamais qu'une seule valeur (`scraper_batiment`), rendant tout KPI de répartition par canal creux. Un second canal réel existe désormais : auto-inscription publique (`dashboard/pages_publiques.py::afficher_devenir_client`, `?vue=devenir_client`, `source='auto_inscription'`), qui enchaîne directement sur le formulaire de qualification existant. Reste à construire : la page dashboard de répartition par canal elle-même (pas encore de table/page dédiée, contrairement aux autres modules ci-dessus), une fois qu'il y aura assez de volume sur les deux canaux pour que ce soit informatif.
